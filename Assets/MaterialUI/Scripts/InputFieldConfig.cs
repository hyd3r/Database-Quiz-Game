		...
		void Update ()
		{
			...
			if (state == 1)    // Activating
			{
				if (animDeltaTime <= animationDuration)
				{
					...
					tempVec3.x = Anim.Quint.Out (placeholderScale, dir, animDeltaTime, animationDuration);
					tempVec3.y =tempVec3.x;
					tempVec3.z =tempVec3.x;
					placeholderRect.localScale = tempVec3;

					Vector2 tempVec2 = placeholderRect.pivot;
					tempVec2.y = Anim.Quint.InOut (placeholderPivot, piv, animDeltaTime, animationDuration);
